...
// TestDatastoreSnapshot_CacheLifetime tests that the snapshot cache respects the configured lifetime.
func TestDatastoreSnapshot_CacheLifetime(t *testing.T) {
	...
	snapshot1, err := handle.DatastoreSnapshot("test-snapshot", cycleState)
	if err != nil {
	...
	snapshot2, err := handle.DatastoreSnapshot("test-snapshot", cycleState)
	if err != nil {
	...
	snapshot3, err := handle.DatastoreSnapshot("test-snapshot", cycleState)
	if err != nil {
